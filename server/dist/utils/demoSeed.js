...
const DEMO_COURSES = [
    {
        ...
        description: 'Master React from the ground up. Learn components, hooks, state management, routing and deployment while building real projects.',
    ...
    },
    {
        ...
        description: 'Everything you need to build secure, scalable backends: routing, middleware, authentication, databases and deployment.',
    ...
    },
    {
        ...
        description: 'A friendly, project-driven introduction to Python. Learn syntax, data structures, files, APIs and automation.',
    ...
    },
    {
        ...
        description: 'Arrays, linked lists, trees, graphs, recursion and dynamic programming — explained visually with interview problems.',
    ...
    },
    {
        ...
        description: 'Go deep into the language that powers the web: closures, prototypes, promises, modules and the modern toolchain.',
    ...
    },
    {
        ...
        description: 'A practical tour of modern AI: how models learn, prompt engineering, embeddings, and building AI-powered apps.',
...
const ensureDemoData = async () => {
    try {
        const demoCount = await prisma_1.prisma.course.count({ where: { isDemo: true } });
        if (demoCount > 0)
            return;
        let instructor = await prisma_1.prisma.user.findUnique({
        ...
        if (!instructor) {
            instructor = await prisma_1.prisma.user.create({
                data: {
                    ...
                    password: await bcryptjs_1.default.hash('demo-instructor-password', 10),
                    role: 'INSTRUCTOR',
                },
            });
        }
        for (const demo of DEMO_COURSES) {
            ...
            const existing = await prisma_1.prisma.course.findUnique({ where: { slug: demo.slug } });
            if (existing)
                continue;
